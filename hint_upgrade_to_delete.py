...
class RecommendAdvice():
    ...
    def RateInfo(self, ticker):
        ...
        R1_ranks = self.service.spreadsheets().values().batchGet(spreadsheetId=stock_report_page,
                                                                             ranges='R1!A:BJ',
                                                                             valueRenderOption='FORMATTED_VALUE',
                                                                             dateTimeRenderOption='FORMATTED_STRING').execute()
        # results_rank_updated = service.spreadsheets().values().batchGet(spreadsheetId=stock_report_page, ranges='R1!A:BJ', valueRenderOption='FORMATTED_VALUE', dateTimeRenderOption='FORMATTED_STRING').execute()
        R1_values = R1_ranks['valueRanges'][0]['values'][1:]

        R2_ranks = self.service.spreadsheets().values().batchGet(spreadsheetId=self.stock_report_page,
                                                                 ranges='R2!A:BJ',
                                                                 valueRenderOption='FORMATTED_VALUE',
                                                                 dateTimeRenderOption='FORMATTED_STRING').execute()
        R2_values = R2_ranks['valueRanges'][0]['values'][1:]

        R3_ranks = self.service.spreadsheets().values().batchGet(spreadsheetId=self.stock_report_page,
                                                                 ranges='R3!A:BJ',
                                                                 valueRenderOption='FORMATTED_VALUE',
                                                                 dateTimeRenderOption='FORMATTED_STRING').execute()
        ...
        for i in R1_values:
            if i[1] == ticker and i[-1] == 1:
                rankin_rate = '1 - высокий интерес (покупать)'
                invest_tactics = 'Локомотив'
            else:
                pass
        ...
        for i in R3_values:
            if i[1] == ticker and i[-1] == 1:
                rankin_rate = '1 - высокий интерес (покупать)'
                invest_tactics = 'Потенциал роста'
            else:
                pass

        for i in R1_values:
            if i[1] == ticker and i[-1] == 2:
